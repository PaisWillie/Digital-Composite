...
const pythonExecutable =
  "/home/ubuntu/gradsight/Digital-Composite/server/myenv/bin/python";
...
async function downloadImage(req, res, bucketname) {
  try {
    ...
    if (year == null || program == null) {
      return res
        .status(400)
        .send("Bad Request. Please provide the year and program");
    }

    const { Body, ContentType } = await compositeService.getImage({
      bucketname,
      year,
      program,
    });
...
exports.uploadImageByYearAndProgram = async (req, res) => {
  try {
    ...
    await compositeService.saveImage({
      bucketname,
      year,
      program,
      file: req.file,
    ...
    const previewFile = {
      ...
      buffer: Buffer.isBuffer(scalingImage)
        ? scalingImage
        : Buffer.from(scalingImage),
    };
    ...
    return res
      .status(200)
      .json({ message: "Image uploaded successfully", data: ocrParsedData });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
};
...
exports.blacklistStudent = async (req, res) => {
  try {
    ...
    const blacklistFile = {
      mimetype: "image/jpeg",
      buffer: Buffer.isBuffer(blacklistStudentImg)
        ? blacklistStudentImg
        : Buffer.from(blacklistStudentImg),
    };
    ...
    const blacklistScaledFile = {
      mimetype: "image/jpeg",
      buffer: Buffer.isBuffer(blacklistScaledImage)
        ? blacklistScaledImage
        : Buffer.from(blacklistScaledImage),
    };
    ...
    await compositeService.saveImage({
      ...
      year,
      program,
      file: blacklistScaledFile,
    });

    return res.status(200).json({ message: "Removed student successfully" });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
};
