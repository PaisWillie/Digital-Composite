...
test.describe('Admin Page Tests', () => {
  ...
  test('Verify Admin Dashboard buttons are visible', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Upload Composite' })).toBeVisible();
    await expect(page.getByRole('button', { name: 'Manage Composites' })).toBeVisible();
    await expect(page.getByRole('button', { name: 'Black List Student' })).toBeVisible();
    await expect(page.getByRole('button', { name: 'View Dashboard Analytics' })).toBeVisible();
  });
  ...
  test('Navigate to Manage Composites Page', async ({ page }) => {
    ...
    await expect(page).toHaveURL('http://localhost:5173/admin/manageCompositesPage');
  });

  test('Navigate to Black List Student Page', async ({ page }) => {
    await page.getByRole('button', { name: 'Black List Student' }).click();
    await expect(page).toHaveURL('http://localhost:5173/admin/blackListStudentPage');
  });
